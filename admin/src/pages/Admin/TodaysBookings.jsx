...
function TodaysBookings() {
  ...
  return (
    <div className="w-full m-5">
      <p className="mb-3 text-lg font-medium">Today's Bookings</p>
...
>
  Download Excel
        ...
        <table className="min-w-full bg-white border border-gray-200">
          ...
          <tbody>
            {bookings.filter((item) => new Date(item.createdAt).toLocaleDateString("en-CA") === today).slice().reverse().map((item, i) => (
                ...
                <td className="  py-3 px-4 border-r">
                  {
                    !item.cancelled ? <div className="flex flex-col items-center justify-center gap-4">
                      ...
                      :
                      <button className="bg-red-700 text-white px-2 py-1 rounded-md ">{item.cancelledBy === "admin"?"Cancelled - Admin":"Cancelled - User"}</button>
                  }
                </td>
                <Modal
          isOpen={modalIsOpen}

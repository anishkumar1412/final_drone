...
function OrdersInProgress() {
  const { aToken, bookings, getAllBokings, cancelAppointment, formatDate, allUsers, updatePilot, updateCoPilot,token } = useContext(AdminContext);
...
const handleDownloadExcel = () => {
  ...
  return (
    <div className="w-full m-5">
      <p className="mb-3 text-lg font-medium">Bookings in Progress</p>
...
>
  Download Excel
        ...
        <table className="min-w-full bg-white border border-gray-200">
          ...
          <tbody>
            {bookings.filter(item => {item.progress === true && item.cancelled!=true && item.workCompleted!=true}).slice().reverse().map((item, i) => (

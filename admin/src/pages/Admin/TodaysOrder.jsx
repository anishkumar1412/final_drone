...
function TodaysOrder() {
  ...
  return (
    <div className="w-full m-5">
      <p className="mb-3 text-lg font-medium">Today's({today})Bookings</p>
      ...
      <div className="bg-white border rounded text-sm min-h-[60vh] max-h-[80vh] overflow-y-scroll">
        <table className="min-w-full bg-white border border-gray-200">
          ...
          <tbody>
            {bookings.filter((item) => new Date(item.createdAt).toISOString().split("T")[0] === today) .slice().reverse().map((item, i) => (

...
function Appointment() {
  const { aToken,token,upcomingBookings,  cancelAppointment,getUpcomingBookings, formatDate, allUsers, updatePilot, updateCoPilot } = useContext(AdminContext);
  ...
  return (
    <div className="w-full m-5">
      <p className="mb-3 text-lg font-medium">Upcoming Bookings</p>
...
>
  Download Excel
        ...
        <table className="min-w-full bg-white border border-gray-200">
          ...
          <tbody>
            {upcomingBookings.slice().reverse().map((item, i) => (
